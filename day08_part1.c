...
void ds_union(DisjointSet *ds, size_t x, size_t y) {
    size_t root_x = ds_find(ds, x);
    size_t root_y = ds_find(ds, y);
    ds_link(ds, root_x, root_y);
}

int component_count_cmp(const void *a, const void *b) {
...
size_t* ds_component_sizes(DisjointSet *ds) {
    size_t *root_sizes = calloc(ds->size, sizeof(size_t));
    if (!root_sizes) return NULL;

    for (size_t i = 0; i < ds->size; i++) {
        size_t root = ds_find(ds, i);
        root_sizes[root]++;
    }

    qsort(root_sizes, ds->size, sizeof(*root_sizes), component_count_cmp);
    return root_sizes;
}
...
int main() {
    ...
    for (int i=0; i<MAX_CONNECTIONS; i++) {
        Pair p = pairs[i];
        ds_union(djs, p.i, p.j);
    }

    size_t *sizes = ds_component_sizes(djs);
